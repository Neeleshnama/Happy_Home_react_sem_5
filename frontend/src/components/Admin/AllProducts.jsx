// import { Button } from "@material-ui/core";
// import { DataGrid } from "@material-ui/data-grid";
// import React, { useEffect } from "react";
// import { AiOutlineDelete, AiOutlineEye } from "react-icons/ai";
// import { useDispatch, useSelector } from "react-redux";
// import { Link } from "react-router-dom";
// import { getAllProductsShop } from "../../redux/actions/product";
// import { deleteProduct } from "../../redux/actions/product";
// import Loader from "../Layout/Loader";
// import axios from "axios";
// import { server } from "../../server";
// import { useState } from "react";

// const AllProducts = () => {
//   const [data, setData] = useState([]);

//   useEffect(() => {
//     axios.get(`${server}/product/admin-all-products`, {withCredentials: true}).then((res) => {
//         setData(res.data.products);
//     })
//   }, []);

//   const columns = [
//     { field: "id", headerName: "Product Id", minWidth: 150, flex: 0.7 },
//     {
//       field: "name",
//       headerName: "Name",
//       minWidth: 180,
//       flex: 1.4,
//     },
//     {
//       field: "price",
//       headerName: "Price",
//       minWidth: 100,
//       flex: 0.6,
//     },
//     {
//       field: "Stock",
//       headerName: "Stock",
//       type: "number",
//       minWidth: 80,
//       flex: 0.5,
//     },

//     {
//       field: "sold",
//       headerName: "Sold out",
//       type: "number",
//       minWidth: 130,
//       flex: 0.6,
//     },
//     {
//       field: "Preview",
//       flex: 0.8,
//       minWidth: 100,
//       headerName: "",
//       type: "number",
//       sortable: false,
//       renderCell: (params) => {
//         return (
//           <>
//             <Link to={`/product/${params.id}`}>
//               <Button>
//                 <AiOutlineEye size={20} />
//               </Button>
//             </Link>
//           </>
//         );
//       },
//     },
//   ];

//   const row = [];

//   data &&
//   data.forEach((item) => {
//       row.push({
//         id: item._id,
//         name: item.name,
//         price: "₹ " + item.discountPrice,
//         Stock: item.stock,
//         sold: item?.sold_out,
//       });
//     });

//   return (
//     <>
//         <div className="w-full mx-8 pt-1 mt-10 bg-blue-100">
//           <DataGrid
//             rows={row}
//             columns={columns}
//             pageSize={10}
//             disableSelectionOnClick
//             autoHeight
//           />
//         </div>
//     </>
//   );
// };

// export default AllProducts;
import React, { useEffect, useState } from "react";
import { Button } from "@material-ui/core";
import { DataGrid } from "@material-ui/data-grid";
import { AiOutlineDelete, AiOutlineEye } from "react-icons/ai";
import { Link } from "react-router-dom";
import axios from "axios";
import { server } from "../../server";


const AllProducts = () => {
  const [data, setData] = useState([]);

  useEffect(() => {
    axios.get(`${server}/product/admin-all-products`, { withCredentials: true }).then((res) => {
      setData(res.data.products);
    });
  }, []);

  const columns = [
    { field: "id", headerName: "Product Id", minWidth: 150, flex: 0.7 },
    {
      field: "name",
      headerName: "Name",
      minWidth: 180,
      flex: 1.4,
    },
    {
      field: "price",
      headerName: "Price",
      minWidth: 100,
      flex: 0.6,
    },
    {
      field: "Stock",
      headerName: "Stock",
      type: "number",
      minWidth: 80,
      flex: 0.5,
    },
    {
      field: "sold",
      headerName: "Sold out",
      type: "number",
      minWidth: 130,
      flex: 0.6,
    },
    {
      field: "Preview",
      flex: 0.8,
      minWidth: 100,
      headerName: "",
      type: "number",
      sortable: false,
      renderCell: (params) => {
        return (
          <>
            <Link to={`/product/${params.id}`}>
              <Button>
                <AiOutlineEye size={20} />
              </Button>
            </Link>
          </>
        );
      },
    },
  ];

  const row = [];

  data &&
    data.forEach((item) => {
      row.push({
        id: item._id,
        name: item.name,
        price: "₹ " + item.discountPrice,
        Stock: item.stock,
        sold: item?.sold_out,
      });
    });

  // Prepare data for VictoryPie
  const pieData = data.map((product) => ({
    x: product.name,
    y: product.sold_out * product.discountPrice, // Total share calculation
  }));

  // Prepare data for VictoryLine
  const lineData = data.map((product) => ({
    x: product.name,
    y: product.discountPrice * product.sold_out,
  }));

  return (
    <>
      {/* <div className="w-full mx-8 pt-1 mt-10 bg-blue-100">
        <DataGrid rows={row} columns={columns} pageSize={10} disableSelectionOnClick autoHeight />
      </div>
      <br />
      <div className="w-full mx-8 pt-1 mt-10">
        <h3 className="text-xl font-semibold">Total Share of Products</h3>
        <VictoryPie
          data={pieData}
          colorScale="qualitative"
          style={{ labels: { fontSize: 12, fill: "white" } }}
        />
      </div>
      <br />
      <div className="w-full mx-8 pt-1 mt-10">
        <h3 className="text-xl font-semibold">Share Over Time</h3>
        <VictoryChart
          containerComponent={<VictoryVoronoiContainer />}
          width={600}
          height={400}
          domainPadding={{ x: 20 }}
        >
          <VictoryAxis tickFormat={() => ""} />
          <VictoryAxis dependentAxis />
          <VictoryLine
            data={lineData}
            style={{
              data: { stroke: "#c43a31" },
              parent: { border: "1px solid #ccc" },
            }}
            labels={({ datum }) => `${datum.x}: ${datum.y}`}
            labelComponent={<VictoryTooltip />}
          />
        </VictoryChart>
      </div> */}
      <div className="w-full mx-8 pt-1 mt-10 bg-blue-100">
        <DataGrid rows={row} columns={columns} pageSize={10} disableSelectionOnClick autoHeight />
      </div>
      
      {/* <div className="w-full mx-8 pt-1 mt-10">
        <h3 className="text-xl font-semibold">Share Over Time</h3>
        <VictoryChart
          containerComponent={<VictoryVoronoiContainer />}
          width={600}
          height={400}
          domainPadding={{ x: 20 }}
        >
          <VictoryAxis tickFormat={() => ""} />
          <VictoryAxis dependentAxis />
          <VictoryLine
            data={lineData}
            style={{
              data: { stroke: "#c43a31" },
              parent: { border: "1px solid #ccc" },
            }}
            labels={({ datum }) => `${datum.x}: ${datum.y}`}
            labelComponent={<VictoryTooltip />}
          />
        </VictoryChart>
      </div> */}
    </>
  );
};

export default AllProducts;
