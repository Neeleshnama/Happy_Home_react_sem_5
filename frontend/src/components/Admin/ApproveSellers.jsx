...
const ApproveSellers = () => {
  ...
  const fetchSellers = async () => {
    try {
      const res = await axios.get(`${server}/shop/all`);
      const sellersWithIds = res.data.map((seller) => ({ ...seller, id: seller._id }));
      setFilteredSellers(sellersWithIds);
    } catch (error) {
      toast.error(error.response.data.message);
    }
  };
  ...
  const handleFilterChange = async (filter) => {
    try {
      let url = `${server}/shop/`;
      if (filter === "verified" || filter === "notverified") {
        url += filter;
      }
  ...
  const handleAction = async (id, action) => {
    try {
      const res = await axios.put(`${server}/shop/${action}-seller/${id}`, { withCredentials: true });
      toast.success(res.data.message);
      fetchSellers(); // Refresh the seller list after action
    } catch (error) {
      toast.error(error.response.data.message);
    }
  };
    ...
    <div className="w-full flex justify-center pt-5 bg-blue-100">
      <div className="w-[97%]">
        ...
        >
          ...
          <option value="notverified">Not Verified Sellers</option>
        </select>
